...
import { ApiCategory, ApiService } from '../services/api.service';
...
export class Dashboard implements OnInit {
  ...
  readonly pagedServices = computed(() => {
    ...
    return this.state.services().slice(start, start + this.PAGE_SIZE);
  });

  readonly totalPages = computed(() =>
    Math.max(1, Math.ceil(this.state.services().length / this.PAGE_SIZE))
  );
  ...
  readonly pageEnd = computed(() => Math.min(this.currentPage() * this.PAGE_SIZE, this.state.services().length));
  ...
  ngOnInit() {
    if (this.state.businesses().length === 0) {
      this.state.loadBusinesses();
    }
  }

  selectCategory(cat: ApiCategory) {
    this.currentPage.set(1);
    this.state.loadServicesForCategory(cat);
  }

  openService(service: ApiService) {
